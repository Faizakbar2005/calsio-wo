<template>
  <div class="tab-cari">
    ...
    <div v-if="!hasSearched && !isSearching" class="idle-state">
      ...
      <p class="idle-sub">Sistem akan mencari di seluruh panduan gerakan, muscle guide, tips nutrisi, dan tabel MET secara otomatis</p>
    </div>
    ...
    <div v-else-if="isSearching" class="searching-state">
      ...
      <p class="searching-label">Mencari & merangkum dengan Gemini...</p>
    </div>
    ...
    <div v-else-if="hasSearched" class="results-wrap">
      ...
      <!-- Summary Card (Gemini) -->
      <div class="summary-card" :class="{ error: summaryError }">
      ...
      <div v-else>
        ...
        <div class="results-grid">
          ...
          >
            ...
            <div
              class="score-bar"
              :style="{ width: (result.score / results[0].score * 100) + '%', background: result.color }"
            ></div>
            ...
            <div class="result-card-inner">
              <div class="result-header">
                ...
                <div class="result-meta">
                  <div class="result-type-badge" :style="typeBadgeStyle(result.type)">
...
<script>
import { search, formatForGemini } from './irService.js'
...
export default {
  ...
  methods: {
    async doSearch() {
      ...
      try {
        // 1. Information Retrieval — cari dokumen relevan
        ...
        // 2. Summarize dengan Gemini (async, tidak block UI)
        if (searchResults.length > 0) {
          this.summarizeWithGemini(searchResults, q)
        } else {
          this.summary = 'Tidak ditemukan informasi yang relevan. Coba kata kunci yang lebih spesifik.'
        }
    ...
    async summarizeWithGemini(results, query) {
      if (!GEMINI_API_KEY) {
        this.summaryError = 'Gemini API key belum dikonfigurasi. Tambahkan VITE_GEMINI_API_KEY di file .env'
        return
      }

      try {
        const prompt = formatForGemini(results, query)
        ...
        this.summary = data.candidates?.[0]?.content?.parts?.[0]?.text
          || 'Tidak ada respons dari Gemini.'

      } catch (err) {
        console.error('Gemini error:', err)
        this.summaryError = `Gagal mendapatkan summary: ${err.message}`
      }
    },
    ...
    getSnippet(result) {
      ...
      if (result.type === 'met')      return `MET ${raw.met} • ${raw.category} • Intensitas ${raw.intensity} • ±${Math.round(raw.met * 70 * 0.5)} kcal/30 menit (70kg)`
      return ''
    },
    ...
    typeBadgeStyle(type) {
      const colors = {
        exercise: 'rgba(234,179,8,0.15)',
        muscle:   'rgba(96,165,250,0.15)',
        nutrisi:  'rgba(34,197,94,0.15)',
        makro:    'rgba(34,197,94,0.15)',
        met:      'rgba(168,139,250,0.15)',
      }
      const textColors = {
        exercise: '#eab308',
        muscle:   '#60a5fa',
      ...
      return {
        ...
        color: textColors[type] || 'rgba(255,255,255,0.5)'
      }
    }
  }
}
...
.search-icon-left {
  padding: 0 14px;
  font-size: 1rem; opacity: 0.4; flex-shrink: 0;
}
.search-input {
...
.summary-error {
  ...
  font-size: 0.85rem; color: #fca5a5; line-height: 1.6;
}
.summary-empty {
...
.results-label {
  ...
  letter-spacing: 0.12em; color: rgba(255,255,255,0.25); margin-bottom: 10px;
}
...
.result-emoji { font-size: 1.6rem; flex-shrink: 0; line-height: 1; }
